...
const message2 = `- 請對所有時段設置志願序(每個時段的上限人數為 50 人)，無特定年級優先。
...
- 參加系上實驗課預選，並不代表 電子學實驗（二）或電路學實驗 會出現在您的課表上，請務必進入學校選課系統點選 電子學實驗（二）或電路學實驗，這樣才算數，期末才會有成績。`;

const message3 = `選擇有興趣的實驗並設置志願序。

- 請先詳細閱讀**十選二實驗規定**。部分課程有相關先修規定。
- **[注意]** 電磁波實驗會針對兩個時段 (週二 9:00 ~ 12:00、週三 9:00 ~ 12:00) 各設置一個志願，請同學考量自身時間狀況進行志願的排序，系統會自行判斷不會同時選上兩個時段之電磁波實驗。
- 可設置 0 ~ 10 個志願序 (不含數電實驗、有兩個電磁波實驗)，所有志願序都有可能選上。
數電實驗另開 Google 表單，3 人組隊報名。表單開放時間與預選系統相同。
- 所有學生至多選上 2 門實驗，包含數電實驗。
- 抽籤作業流程：(A) 數電實驗抽籤 → (B) 志願序抽籤

十選二實驗規定：[https://reurl.cc/77ebG5](https://reurl.cc/77ebG5)`

const message4 = `通常是全部選上，若遇到需要抽籤情況，抽籤方式為**隨機**。

數電實驗表單：[https://forms.gle/ysa25cSnCuiwby3v5](https://forms.gle/ysa25cSnCuiwby3v5)`;
...
- 本學期**網路與多媒體**設有**特定年級優先**；**光電**設置**年級保障名額**。`
...
5. 在產生課程選項志願的時候，會從優先度高的人開始抽。若有一群學生優先度相同則 random 排序進行分。
